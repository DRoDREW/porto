...
document.querySelectorAll('a, button').forEach(el => {
  el.addEventListener('mouseenter', () => {
    ring.style.width = '56px'; ring.style.height = '56px'; ring.style.opacity = '0.2';
  });
  el.addEventListener('mouseleave', () => {
    ring.style.width = '36px'; ring.style.height = '36px'; ring.style.opacity = '0.4';
  });
});

/* ═══════════════════════════════
   SPLASH + MAGNET LINES (SPLASH)
═══════════════════════════════ */
(function initSplash() {
...
(function initHeroMagnet() {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      lines.push({ x: (c + 0.5) * cw, y: (r + 0.5) * ch, angle: 0 });
    }
  }
  ...
  function draw() {
    ...
    lines.forEach((l, i) => {
      ...
      ctx.rotate(targets[i]);
      ctx.fillStyle = '#C8A96E';
      ctx.globalAlpha = 0.5;
      ctx.beginPath();
      ctx.roundRect(-1, -16, 2, 32, 2);
      ctx.fill();
      ctx.restore();
    });
...
(function initVelocity() {
  ...
  function fillTrack(id, text, direction) {
    ...
    let html = '';
    for (let i = 0; i < 4; i++) {
      html += `<span class="velocity-item">${text}</span>`;
    }
    track.innerHTML = html;

    let pos = 0;
    const speed = direction > 0 ? 0.5 : -0.5;
  ...
  try {
    ...
    weeks.forEach((week, wi) => {
      ...
      week.forEach((day, di) => {
        ...
        html += `<div class="cal-cell ${cls}" 
          style="opacity:0;animation:fadeIn 0.4s ease ${delay}s forwards;"
          data-date="${day.date}" data-count="${day.contributionCount}"></div>`;
      });
      html += '</div>';
    });
    html += '</div>';
    ...
    container.querySelectorAll('.cal-cell').forEach(cell => {
      cell.addEventListener('mouseenter', e => {
        ...
        tooltip.textContent = `${c} contributions on ${d}`;
...
(function initCards() {
  ...
  const LANE_H = window.innerHeight;
  ...
  projects.forEach((p, i) => {
    ...
    if (i === N - 1) lane.style.height = '0px';
    container.appendChild(lane);
    laneEls.push(lane);

    const sticky = document.createElement('div');
    sticky.className = 'card-sticky';
    lane.appendChild(sticky);

    const card = document.createElement('div');
    card.className = 'project-card';
    card.style.rotate = ROTATIONS[i % ROTATIONS.length] + 'deg';
    card.style.marginTop = (i * STACK_OFFSET) + 'px';
  ...
  laneEls[N - 1].style.height = LANE_H + 'px';
  ...
  function tick() {
    cardEls.forEach((card, i) => {
      const nextLane = laneEls[i + 1];
      if (!nextLane) return;

      const nextRect = nextLane.getBoundingClientRect();
      const raw      = 1 - Math.max(0, Math.min(1, nextRect.top / LANE_H));
      const scale    = 1 - SCALE_RANGE * raw;
      const translateY = -20 * raw;

      card.style.transform = `scale(${scale.toFixed(4)}) translateY(${translateY.toFixed(2)}px)`;
      card.style.filter = `brightness(${(1 - 0.15 * raw).toFixed(3)})`;
    });
